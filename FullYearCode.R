...
png(paste0("out/fig_", file_id, ".png"), width = 900 * 3.5, height = 700 * 3.5, res = 300)
par(mfrow = c(2, 2))
plot(cb_tvar_reduce, 
...
for (cold_heat in c("cold", "heat")) {
  ...
  plot(cb_lag_reduce,
       main = paste0(ifelse(cold_heat == "cold", "b", "c"), ") Lag structure at ", get(paste0("val_", cold_heat)) * 100, "th percentile"),
       xlab = "Lag (in days)",
       ylab = "Odds ratio",
       lwd  = 1.5,
  )
